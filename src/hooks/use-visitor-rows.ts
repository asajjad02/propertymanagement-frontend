 ...
 * flat number (resolved from reference lookups) and any vehicle registered
 * against the visit. Status is derived — a visitor with an exit_time is
 * "checked out", otherwise "inside".
 */
...
/** Server-filtered/paginated/sorted visitors, joined with flat/vehicle. */
export function useVisitorRows(listParams: ListParams) {
  ...
  }, [visitors.data, vehicles.data, flats.map]);

  return {
    rows,
    count: visitors.data?.count ?? 0,
    isLoading: visitors.isPending || vehicles.isPending || flats.isPending,
